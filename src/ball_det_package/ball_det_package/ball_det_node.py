...
class BallDetectorNode(Node):
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(BallArray, 'balls_in_image', 10)
        self.get_logger().info(f"Publisher created for topic: {self.publisher_.topic}")
        ...
        if circles is not None:
            ...
            for i in circles[0, :]:
                ...
                if field_ratio > 0.16 and ball_ratio < 0.47:
                    ...
                    ball = Ball()
                    ball.center.x = float(x_new)
                    ball.center.y = float(y_new)

                    ball.bb.center.position.x = float(x_new)
                    ball.bb.center.position.y = float(y_new)
                    ball.bb.center.theta = 0.0 
                    ball.bb.size_x = float(r_new * 2)
                    ball.bb.size_y = float(r_new * 2)

                    ball.confidence.confidence = 1.0

                    ball_array_msg = BallArray()
                    ball_array_msg.header.stamp = self.get_clock().now().to_msg()
                    ball_array_msg.header.frame_id = "camera"  # sesuaikan jika perlu
                    ball_array_msg.balls = [ball]

                    self.publisher_.publish(ball_array_msg)
                    self.get_logger().info(f'Published ball at x: {ball.center.x}, y: {ball.center.y}')
                else:
                    continue
