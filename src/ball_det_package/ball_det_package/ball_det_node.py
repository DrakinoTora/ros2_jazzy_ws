...
class BallDetectorNode(Node):
    ...
    def detect_ball(self):
        ...
        if circles is not None:
            ...
            hline = mask_ball[line_y, line_x_start:line_x_end]
            nonzero = np.where(hline == 255)[0]

            if len(nonzero) < 2:
                return

            first = line_x_start + nonzero[0]
            last = line_x_start + nonzero[-1]
            total_x_pixel = last - first
            if total_x_pixel <= 0:
                return

            actual_diameter = 0.13  # meter
            focal_length = 714.1    # pixel
            distance = (actual_diameter * focal_length) / total_x_pixel
